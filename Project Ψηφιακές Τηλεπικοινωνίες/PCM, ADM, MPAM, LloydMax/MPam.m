function [OutputBitMatrix, OutputSymbolVector, InputSymbolVector, InputBitMatrix] = MPam(N, M, EncodingType, SNR)
    ...
    % Encode the binary matrix to decimal vector
    % Element: Decimal representation of the symbol
    DecimalVector = bi2de(BitMatrix);
    
    % If the encoding type is Gray..
    if EncodingType == "Gray"
        % Encode the decimal symbols to Gray decimal symbols
        SymbolVector = bin2gray(DecimalVector, 'pam', M);
    else
        % Use the simple encoded decimal symbols
        SymbolVector = DecimalVector;
    end
    ...
    % If the encoding type is Gray...
    if EncodingType == "Gray"
        % Decode the gray decimal symbols to decimal symbols
        DecimalVector = gray2bin(SymbolVector, 'pam', M);
    else
        % Use the decimal symbols
        DecimalVector = SymbolVector;
    end
	
    % Decode the decimal symbols to bits
    OutputBitMatrix = de2bi(DecimalVector);
end