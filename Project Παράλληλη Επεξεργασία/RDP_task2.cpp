...
void RamerDouglasPeucker(const vector<Point> &pointList, double epsilon, vector<Point> &out)
{
	...
	if (dmax > epsilon) {
		...
		#pragma omp task shared(epsilon, recResults1) // Create task for the first line.
		{
			RamerDouglasPeucker(firstLine, epsilon, recResults1); // Recursive call-task creation, for the first line.
		}
		#pragma omp task shared(epsilon, recResults2) // Create task for the second line.
		{
		RamerDouglasPeucker(lastLine,  epsilon, recResults2); // Second line recursive call by the current task.
		}
		// Build the result list
		#pragma omp taskwait // Wait for all the child task(recursively) to finish, then create the simplified polyline.
...
int main(int argc, char *argv[])
{
	...
	for (vector< vector<Point> >::const_iterator it1 = AllPolylines.begin(); it1 != AllPolylines.end(); it1++) {
		...
		#pragma omp parallel shared(epsilon) private(simplified_polyline) firstprivate(polyline) // Parallel section
		{
			#pragma omp single nowait // Only one thread can run this section. No barrier.
			{
