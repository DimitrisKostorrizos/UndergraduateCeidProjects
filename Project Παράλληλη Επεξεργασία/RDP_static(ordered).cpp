...
int main(int argc, char *argv[])
{
	...
	#pragma omp parallel shared(epsilon) // parallel section
	{	
		double execution_time = omp_get_wtime(); // thread starting time
		#pragma omp for schedule(static) private(polyline,simplified_polyline) ordered // static scheduling, chunk_size = Number_of_Loops/Number_of_Threads
		for (vector< vector<Point> >::const_iterator it1 = AllPolylines.begin(); it1 < AllPolylines.end(); it1++) 
		{
			...
			#pragma omp ordered // ordered push_back of the simplified_polyline
			SimplifiedAllPolylines.push_back(simplified_polyline);
		}
		execution_time = omp_get_wtime() - execution_time;// thread execution time
		printf( "Time taken by thread %d is %f\n", omp_get_thread_num(), execution_time);
	}
