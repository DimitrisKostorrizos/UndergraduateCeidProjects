...
#pragma omp declare reduction (merge : std::vector<vector<Point>> : omp_out.insert(omp_out.end(), omp_in.begin(), omp_in.end()))
...
int main(int argc, char *argv[])
{
	...
	#pragma omp parallel for schedule(static) reduction(merge:SimplifiedAllPolylines) shared(epsilon) private(polyline,simplified_polyline) firstprivate(execution_time)
